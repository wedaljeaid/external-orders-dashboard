...
function render(rows,filters){
  ...
  const dataType=showRejectedReasons?countByLabel(rows.map((r)=>mapRejectedReason(r.dataType)).filter((v)=>v)):showWaitingType?countByLabel(rows.filter((r)=>r.dataType==="معلق في انتظار تحديد نوع البيانات").map(()=>"طلب البيانات غير واضح")):countBy(rows.filter((r)=>!NON_DATA_TYPES.has(r.dataType)),"dataType");
...
function filterRowsState(rows,filters,ignore){ignore=ignore||[];return rows.filter((r)=>{if(!ignore.includes("startDate")&&filters.startDate&&(!r.requestDate||r.requestDate<filters.startDate))return false;if(!ignore.includes("endDate")&&filters.endDate&&(!r.requestDate||r.requestDate>filters.endDate))return false;if(!ignore.includes("entity")&&filters.entity!=="الكل"&&r.entityType!==filters.entity)return false;if(!ignore.includes("status")&&filters.status!=="الكل"&&r.status!==filters.status)return false;if(!ignore.includes("purpose")&&filters.purpose!=="الكل"&&r.purpose!==filters.purpose)return false;if(!ignore.includes("dataType")&&filters.dataType!=="الكل"&&((filters.status==="مرفوض"?mapRejectedReason(r.dataType):filters.status==="معلق عند مقدم الطلب"?mapWaitingReason(r.dataType):r.dataType)!==filters.dataType))return false;if(!ignore.includes("department")&&filters.department!=="الكل"&&r.department!==filters.department)return false;return true;});}
function allowedDataTypes(rows,filters){if(filters.status==="مرفوض") return rows.map((r)=>mapRejectedReason(r.dataType)).filter((v)=>v); if(filters.status==="معلق عند مقدم الطلب") return rows.map((r)=>mapWaitingReason(r.dataType)).filter((v)=>v); return rows.map((r)=>r.dataType).filter((v)=>v&&v!=="غير محدد").filter((v)=>!NON_DATA_TYPES.has(v));}
...
function mapWaitingReason(value){const v=txt(value); return v.includes("معلق في انتظار تحديد نوع البيانات")?"طلب البيانات غير واضح":"";}
